...
// Parallax effect with requestAnimationFrame debounce
let lastScrollY = 0;
let ticking = false;
function doParallax() {
  const scrollY = window.scrollY;
  document.querySelector('.parallax-bg').style.transform = `translateY(${scrollY * 0.12}px)`;
  document.querySelector('.parallax-mid').style.transform = `translateY(${scrollY * 0.25}px)`;
  document.querySelector('.parallax-fg').style.transform = `translateY(${scrollY * 0.45}px)`;
  document.querySelector('.parallax-rover').style.transform = `translateY(${scrollY * 0.7}px)`;
  ticking = false;
}
...
const subtitleText = "The only limit is our imagination. Let's build Mars together.";
let typeIdx = 0;
function typeSubtitle() {
  if (typeIdx <= subtitleText.length) {
    heroSubtitle.textContent = subtitleText.slice(0, typeIdx);
    typeIdx++;
    setTimeout(typeSubtitle, 36 + Math.random()*60);
  }
}
setTimeout(typeSubtitle, 900);
